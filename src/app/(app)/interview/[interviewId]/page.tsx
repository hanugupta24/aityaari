...
import {
  ...
  type AnalyzeInterviewFeedbackInput,
} from "@/ai/flows/analyze-interview-feedback";
...
const MAX_TAB_SWITCH_WARNINGS = 2; // Interview ends on the 3rd infraction
const FACE_DETECTION_INTERVAL_MS = 5 * 1000; // Check every 5 seconds for face
const CONSECUTIVE_NO_FACE_CHECKS_TO_WARN_FACE_AWAY = 1; // 1 failed check (~5s) triggers strict warning
...
const DISTRACTION_CHECK_INTERVAL_MS = 3 * 1000; // Check every 3 seconds for simulated distraction
const DISTRACTION_PROBABILITY = 0.5; // 50% chance

type ProctoringIssueType =
  | "tabSwitch"
  | "faceNotDetected_short"
  | "task_inactivity"
  | "distraction";
type OnScreenWarning = {
  type: ProctoringIssueType | "none";
  message: string | null;
};
...
export default function InterviewPage() {
  ...
  const [hasSpokenCurrentQuestion, setHasSpokenCurrentQuestion] =
    useState(false);
  ...
  const [
    continuousFaceNotDetectedStartTime,
    setContinuousFaceNotDetectedStartTime,
  ] = useState<number | null>(null);
  const [lastActivityTime, setLastActivityTime] = useState<number>(Date.now());

  const [onScreenProctoringWarning, setOnScreenProctoringWarning] =
    useState<OnScreenWarning>({ type: "none", message: null });
  ...
  const stopMediaTracks = useCallback(() => {
    ...
    if (videoRef.current && videoRef.current.srcObject) {
      ...
      console.log(
        "InterviewPage: Video element srcObject tracks stopped and cleared."
      );
    }
  }, []);
  ...
  const handleEndInterview = useCallback(
    async (
      ...
      const currentTranscript =
        transcriptStringParam || transcriptLogRef.current.join("\\n");
      ...
      if (
        ...
        if (!isEndingInterviewRef.current && user && session) {
          console.log(
            "InterviewPage: handleEndInterview - User or session data missing or already ending. Skipping."
          );
        } else if (isEndingInterviewRef.current)
          console.log(
            "InterviewPage: handleEndInterview - Already ending, skipping."
          );
        return;
      }

      console.log(
        `InterviewPage: handleEndInterview called with reason: ${reason}`
      );
      ...
      switch (reason) {
        case "time_up":
          toastTitle = "Time's Up!";
          toastMessage = "The interview time has concluded.";
          toastVariant = "destructive";
          break;
        case "prolonged_face_absence":
          toastTitle = "Proctoring Alert";
          toastMessage =
            "Interview terminated due to prolonged absence from screen.";
          toastVariant = "destructive";
          break;
        case "all_questions_answered":
          toastTitle = "Interview Complete";
          break;
        case "completed_by_user":
          toastTitle = "Interview Submitted";
          break;
        case "tab_switch_limit":
          toastTitle = "Proctoring Alert";
          toastMessage = "Interview terminated due to excessive tab switching.";
          toastVariant = "destructive";
          break;
        case "face_not_detected_limit":
          toastTitle = "Proctoring Alert";
          toastMessage =
            "Interview terminated due to face not being consistently visible.";
          toastVariant = "destructive";
          break;
      }
      ...
      try {
        ...
        refreshUserProfile().catch((err) =>
          console.error(
            "AuthContext: Error refreshing user profile after interview:",
            err
          )
        );
        ...
        const feedbackInput: AnalyzeInterviewFeedbackInput = {
          ...
          candidateProfile: `Field: ${userProfile.profileField}, Role: ${
            userProfile.role
          }, Education: ${userProfile.educationHistory}, Skills: ${
            userProfile.keySkills || "Not specified"
          }`,
        ...
        console.log(
          "InterviewPage: AI feedback stored. Navigating to feedback page."
        );
        router.push(`/feedback/${interviewId}`);
      } catch (error: any) {
        console.error(
          "InterviewPage: Error ending interview / getting feedback:",
          error
        );
        toast({
          title: "Error Finalizing Interview",
          description:
            error.message ||
            "Failed to finalize and get feedback. Please check your past interviews later.",
          variant: "destructive",
          duration: 10000,
        });
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    console.log(
      `InterviewPage: Main useEffect - Setting up for user ${user.uid}, interview ${interviewId}`
    );
    ...
    try {
      unsubscribe = onSnapshot(
        ...
        (docSnap) => {
          if (isEndingInterviewRef.current) {
            console.log(
              "InterviewPage: Firestore listener - interview ending, skipping snapshot update."
            );
            return;
          }
          if (docSnap.exists()) {
            ...
            if (data.status === "completed" || data.status === "cancelled") {
              if (!isEndingInterviewRef.current) {
                console.log(
                  "InterviewPage: Firestore listener - Session already completed/cancelled. Redirecting."
                );
                toast({
                  title: "Interview Ended",
                  description: "This interview session is no longer active.",
                });
                router.push("/dashboard");
              }
              return;
            ...
            if (data.questions && data.questions.length > 0) {
              const sortedQuestions = data.questions
                ...
                .sort((a, b) => {
                  if (a.stage === "oral" && b.stage === "technical_written")
                    return -1;
                  if (a.stage === "technical_written" && b.stage === "oral")
                    return 1;
              ...
              toast({
                title: "Error",
                description:
                  "No questions found for this session. Please start a new interview.",
                variant: "destructive",
              });
              if (!isEndingInterviewRef.current)
        ...
        (error) => {
          ...
          toast({
            ...
            description: "Could not listen to interview updates.",
            variant: "destructive",
          });
          setIsLoading(false);
        }
      ...
      console.error(
        "InterviewPage: Exception setting up Firestore listener:",
        e
      );
      setIsLoading(false);
    }
    ...
    navigator.mediaDevices
      ...
      .catch((err) => {
        ...
        toast({
          title: "Media Error",
          description:
            "Could not access camera/microphone. Proctoring features may be limited.",
          variant: "destructive",
        });
      });
  ...
  useEffect(() => {
    ...
    if (SpeechRecognitionAPI) {
      ...
      console.warn(
        "InterviewPage: Speech recognition API not supported by this browser."
      );
      toast({
        title: "Speech Input Not Supported",
        description:
          "Your browser doesn't support speech-to-text. You may need to type answers for oral questions.",
        variant: "default",
        duration: 7000,
      });
    }
    ...
    const loadVoices = () => {
      if (typeof window !== "undefined" && window.speechSynthesis) {
        ...
        if (voices.length > 0) {
          ...
          console.log(
            "InterviewPage: Speech synthesis voices loaded:",
            voices.length
          );
  ...
  useEffect(() => {
    if (
      ...
      if (typeof window !== "undefined" && window.speechSynthesis) {
        if (window.speechSynthesis.speaking || window.speechSynthesis.pending) {
          console.log(
            "InterviewPage: AI was already speaking or pending, cancelling before new utterance for question:",
            currentQuestion.id
          );
          window.speechSynthesis.cancel();
        }
        ...
        let desiredVoice = availableVoices.find(
          (voice) =>
            voice.lang.startsWith("en-IN") &&
            voice.name.toLowerCase().includes("female")
        );
        if (!desiredVoice)
          desiredVoice = availableVoices.find((voice) =>
            voice.lang.startsWith("en-IN")
          );
        if (!desiredVoice)
          desiredVoice = availableVoices.find(
            (voice) =>
              voice.lang === "en-US" &&
              voice.name.toLowerCase().includes("female")
          );
        ...
        if (desiredVoice) {
          ...
          console.log(
            "InterviewPage: Using voice for AI dictation:",
            desiredVoice.name,
            desiredVoice.lang
          );
        } else {
          console.log(
            "InterviewPage: No specific Indian accent or 'en-US Female/Google' voice found, using browser default for AI dictation."
          );
        }
        ...
        utterance.onstart = () => {
          ...
          console.log(
            "InterviewPage: AI speech started for question:",
            currentQuestion.id
          );
        };
        utterance.onend = () => {
          ...
          console.log(
            "InterviewPage: AI speech ended for question:",
            currentQuestion.id
          );
        };
        utterance.onerror = (event) => {
          // console.error('InterviewPage: SpeechSynthesis Error. Utterance text was:', `"${utterance.text}"`, 'Error event:', event, `If error event is empty, this may indicate a browser/OS level issue with speech synthesis, or no available voices. User has been notified via toast.`);
          setIsAISpeaking(false);
          setHasSpokenCurrentQuestion(true);
          toast({
            title: "AI Voice Error",
            description: "Could not play AI voice. Please read the question.",
            variant: "destructive",
          });
        };
        ...
        try {
          console.log(
            "InterviewPage: Attempting to speak utterance for question:",
            currentQuestion.id,
            "Text:",
            utterance.text
          );
          ...
          console.error(
            "InterviewPage: SYNC EXCEPTION during window.speechSynthesis.speak():",
            e,
            "Utterance text was:",
            `"${utterance.text}"`
          );
          ...
          toast({
            title: "AI Voice Error",
            description:
              "Could not initiate AI voice. Please read the question.",
            variant: "destructive",
          });
        }
      }
      ...
      isAISpeaking &&
      (!isInterviewEffectivelyActive() ||
        (currentQuestion && currentQuestion.stage !== "oral") ||
        (currentQuestion && currentQuestion.answer) ||
        hasSpokenCurrentQuestion)
    ) {
      if (typeof window !== "undefined" && window.speechSynthesis?.speaking) {
        console.log(
          "InterviewPage: Conditions for AI speaking no longer met, cancelling speech."
        );
        window.speechSynthesis.cancel();
      }
      setIsAISpeaking(false);
  ...
  useEffect(() => {
    if (
      ...
      setLastActivityTime(Date.now());
      console.log(
        `InterviewPage: Interview timer initialized to ${
          session.duration * 60
        } seconds.`
      );
    }
  ...
  const showOnScreenWarning = useCallback(
    (type: ProctoringIssueType, message: string) => {
      ...
      console.log(
        `InterviewPage: Showing on-screen warning: ${type} - ${message}`
      );

      onScreenWarningTimerRef.current = setTimeout(() => {
        setOnScreenProctoringWarning({ type: "none", message: null });
        console.log(
          "InterviewPage: On-screen warning timer expired, clearing warning."
        );
        onScreenWarningTimerRef.current = null;
  ...
  const logProctoringEvent = useCallback(
    (type: ProctoringIssueType) => {
      if (!isInterviewEffectivelyActive() || isEndingInterviewRef.current)
        return;
  ...
  const prevFaceNotDetectedShortCountRef = useRef(
    proctoringIssues.faceNotDetected_short
  );
  ...
  useEffect(() => {
    ...
    const currentCount = proctoringIssues.tabSwitch;
    if (currentCount > prevTabSwitchCountRef.current) {
      console.log(
        `InterviewPage: Tab switch detected (simple warning). Count: ${currentCount}`
      );
      showOnScreenWarning(
        "tabSwitch",
        "Reminder: Please stay focused on the interview tab."
      );
    }
    prevTabSwitchCountRef.current = currentCount;
  }, [
    proctoringIssues.tabSwitch,
    isInterviewEffectivelyActive,
    showOnScreenWarning,
  ]);
  ...
  useEffect(() => {
    if (!isInterviewEffectivelyActive() || isEndingInterviewRef.current) return;
    const currentCount = proctoringIssues.faceNotDetected_short;
    if (currentCount > prevFaceNotDetectedShortCountRef.current) {
      console.log(
        `InterviewPage: Short face absence warning (simple warning). Count: ${currentCount}`
      );
      showOnScreenWarning(
        "faceNotDetected_short",
        "Reminder: Please ensure your face is visible to the camera."
      );
    }
    prevFaceNotDetectedShortCountRef.current = currentCount;
  }, [
    proctoringIssues.faceNotDetected_short,
    isInterviewEffectivelyActive,
    showOnScreenWarning,
  ...
  useEffect(() => {
    if (!isInterviewEffectivelyActive() || isEndingInterviewRef.current) return;
    const currentCount = proctoringIssues.task_inactivity;
    if (currentCount > prevTaskInactivityCountRef.current) {
      console.log(
        `InterviewPage: Task inactivity warning (simple warning). Count: ${currentCount}`
      );
      showOnScreenWarning(
        "task_inactivity",
        "Reminder: Please continue with your answer."
      );
    }
    prevTaskInactivityCountRef.current = currentCount;
  }, [
    proctoringIssues.task_inactivity,
    isInterviewEffectivelyActive,
    showOnScreenWarning,
  ]);
  ...
  useEffect(() => {
    if (!isInterviewEffectivelyActive() || isEndingInterviewRef.current) return;
    const currentCount = proctoringIssues.distraction;
    if (currentCount > prevDistractionCountRef.current) {
      console.log(
        `InterviewPage: Distraction warning (simple warning). Count: ${currentCount}`
      );
      showOnScreenWarning(
        "distraction",
        "Proctoring: Attention Reminder - Please try to maintain focus on the screen and avoid excessive movement or looking away."
      );
    }
    prevDistractionCountRef.current = currentCount;
  }, [
    proctoringIssues.distraction,
    isInterviewEffectivelyActive,
    showOnScreenWarning,
  ]);
  ...
  const handleFocusChange = useCallback(() => {
    if (!isEndingInterviewRef.current && isInterviewEffectivelyActive()) {
      console.log(
        "InterviewPage: Window/tab focus lost or changed. Logging tabSwitch event."
      );
    ...
    if (isInterviewEffectivelyActive()) {
      faceDetectionInterval = setInterval(() => {
        ...
        if (!facePresent) {
          ...
          if (
            consecutiveNoFaceChecksRef.current >=
            CONSECUTIVE_NO_FACE_CHECKS_TO_WARN_FACE_AWAY
          ) {
            logProctoringEvent("faceNotDetected_short");
            consecutiveNoFaceChecksRef.current = 0;
          }
          if (continuousFaceNotDetectedStartTime === null) {
            console.log(
              "InterviewPage: Continuous face not detected timer started."
            );
          ...
          if (continuousFaceNotDetectedStartTime !== null) {
            console.log(
              "InterviewPage: Continuous face not detected timer reset (face now visible)."
            );
          }
  ...
  useEffect(() => {
    if (
      isInterviewEffectivelyActive() &&
      continuousFaceNotDetectedStartTime !== null
    ) {
      const durationAway = Date.now() - continuousFaceNotDetectedStartTime;
      if (durationAway > PROLONGED_FACE_ABSENCE_TIMEOUT_MS) {
        if (!isEndingInterviewRef.current) {
          console.log(
            "InterviewPage: Terminating interview due to prolonged face absence (more than 1 min)."
          );
          handleEndInterview(
            "prolonged_face_absence",
            allQuestionsRef.current,
  ...
  useEffect(() => {
    ...
    if (isInterviewEffectivelyActive() && currentQuestion) {
      const timeoutDuration =
        currentQuestion.stage === "oral"
          ? INACTIVITY_TIMEOUT_ORAL_MS
          : INACTIVITY_TIMEOUT_WRITTEN_MS;
      inactivityInterval = setInterval(() => {
        ...
        if (Date.now() - lastActivityTime > timeoutDuration) {
          console.log(
            "InterviewPage: Task inactivity detected, logging event for simple warning."
          );
    ...
    if (isInterviewEffectivelyActive()) {
      distractionInterval = setInterval(() => {
        ...
        if (Math.random() < DISTRACTION_PROBABILITY) {
          console.log(
            "InterviewPage: Simulated distraction detected, logging event for simple warning."
          );
  ...
  const handleToggleListening = useCallback(() => {
    if (!speechRecognitionSupported) {
      toast({
        title: "Speech Input Not Supported",
        description:
          "Cannot start voice input. Please type your answer if possible or contact support.",
        variant: "destructive",
      });
      return;
    }
    if (isListening) {
      ...
      if (!recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event: any) => {
          ...
          const currentAnswerDisplay = (
            finalTranscriptAggregated.trim() +
            (interimTranscript
              ? (finalTranscriptAggregated.trim() ? " " : "") +
                interimTranscript
              : "")
        ...
        recognitionRef.current.onerror = (event: any) => {
          ...
          if (event.error === "no-speech")
            errorMsg =
              "No speech detected. Please ensure your microphone is working and try again.";
          if (event.error === "audio-capture")
            errorMsg =
              "Audio capture failed. Please check microphone permissions and hardware.";
          ...
          setIsListening(false);
          console.error(
            "InterviewPage: Speech recognition error:",
            event.error,
            errorMsg
          );
        };
      ...
      try {
        ...
        setSpeechError(
          "Failed to start voice input. Please ensure microphone access is granted."
        );
        setIsListening(false);
      }
    }
  ...
  const handleNextQuestion = useCallback(async () => {
    if (
      !user ||
      !session ||
      !allQuestionsRef.current.length ||
      currentQuestionIndex >= allQuestionsRef.current.length ||
      isEndingInterviewRef.current
    ) {
      console.warn(
        "InterviewPage: handleNextQuestion - Preconditions not met or interview ending."
      );
      return;
    }
    ...
    const updatedTranscriptLogArray = [
      ...transcriptLogRef.current,
      ...newTranscriptLogEntries,
    ];
    ...
    try {
      ...
      await updateDoc(sessionDocRef, {
        questions: updatedQuestions.map(({ answer, ...q }) => ({
          ...q,
          answer: answer || "",
        })),
      ...
      toast({
        title: "Save Error",
        description: "Could not save your answer to the database.",
        variant: "destructive",
      });
    }
    ...
    if (nextQuestionIndex < allQuestionsRef.current.length) {
      ...
      console.log(
        `InterviewPage: Moving to question ${nextQuestionIndex + 1}.`
      );
    } else {
      ...
      await handleEndInterview(
        "all_questions_answered",
        updatedQuestions,
        updatedTranscriptLogArray.join("\\n")
      );
    }
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    if (currentQuestion?.stage === "technical_written") {
      ...
      toast({
        ...
        description:
          "Please type your answer directly for technical questions.",
        variant: "default",
        duration: 3000,
      });
    }
    ...
    return (
      <div className="flex flex-col justify-center items-center h-screen p-4">
        <Alert variant="destructive" className="max-w-lg">
          ...
          <AlertDescription>
            Problem loading questions or session ended unexpectedly.
          </AlertDescription>
        </Alert>
  ...
  const isSubmitButtonDisabled =
    ...
    isAISpeaking ||
    isListening ||
    (currentStage === "technical_written" && !userAnswer.trim());
