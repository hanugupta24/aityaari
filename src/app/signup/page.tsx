...
export default function SignupPage() {
    ...
    try {
      ...
      const verifier = new RecaptchaVerifier(auth, containerId, {
        ...
        "expired-callback": () => {
          ...
          if (recaptchaVerifierRef.current) {
            // Check if it's the current verifier
            try {
              recaptchaVerifierRef.current.clear();
            } catch (e) {
      ...
      if (recaptchaVerifierRef.current) {
        // If verifier was assigned then errored
        try {
          recaptchaVerifierRef.current.clear();
        } catch (e) {
          /*ignore*/
  ...
  useEffect(() => {
    ...
    console.log(
      `${pageName}: Main reCAPTCHA useEffect. AuthMethod: ${authMethod}, Auth: ${!!auth}, recaptchaContainerRef.current:`,
    ...
    if (authMethod === "phone" && auth) {
      ...
      if (recaptchaContainerRef.current) {
        if (
          ...
          console.log(
            `${pageName}: Conditions met (container ref available, no active verifier, not initializing, not ready). Triggering initRecaptcha for ID: ${RECAPTCHA_CONTAINER_ID_SIGNUP}`
          );
          initRecaptcha(RECAPTCHA_CONTAINER_ID_SIGNUP);
        } else {
          console.log(
            `${pageName}: Not calling initRecaptcha. VerifierRef: ${!!recaptchaVerifierRef.current}, Initializing: ${isRecaptchaInitializing}, Ready: ${isRecaptchaReady}`
          );
        }
      } else {
        console.warn(
          `${pageName}: reCAPTCHA container ref (recaptchaContainerRef.current) NOT available yet for ID ${RECAPTCHA_CONTAINER_ID_SIGNUP}. Will re-run when ref updates or authMethod changes.`
        );
      ...
      console.log(
        `${pageName}: Auth method is not 'phone' or auth not ready. Running cleanup.`
      );
      if (recaptchaVerifierRef.current) {
        ...
        try {
          ...
          console.warn(`${pageName}: Error clearing reCAPTCHA:`, e);
        }
        recaptchaVerifierRef.current = null;
      }
      setIsRecaptchaReady(false);
    ...
    return () => {
      console.log(
        `${pageName}: useEffect cleanup - Current Verifier:`,
        !!recaptchaVerifierRef.current
      );
      if (recaptchaVerifierRef.current) {
        ...
        try {
          ...
          console.warn(`${pageName}: Cleanup - Error clearing verifier:`, e);
        }
        recaptchaVerifierRef.current = null;
      }
  ...
  const handleVerifyOtpAndSignup = async () => {
    ...
    try {
      ...
      const firebaseUser = userCredential.user;
      console.log(
        `${pageName}: Phone number verified, user signed up/in:`,
        firebaseUser.uid
      );
      ...
      if (phoneEmail && phonePassword && firebaseUser) {
        try {
          console.log(
            `${pageName}: Attempting to link email ${phoneEmail} to user ${firebaseUser.uid}`
          );
          ...
          console.log(
            `${pageName}: Email ${phoneEmail} successfully linked to user ${firebaseUser.uid}`
          );
        } catch (linkError: any) {
          console.error(
            `${pageName}: Error linking email credential:`,
            linkError
          );
          ...
          if (linkError.code === "auth/email-already-in-use") {
            ...
            linkErrorMessage =
              "This email/password is already linked to another account or this phone.";
          }
          toast({
            title: "Email Link Error",
            description: linkErrorMessage,
            variant: "destructive",
          });
        }
      }
      ...
      console.log(
        `${pageName}: User profile created/updated in Firestore for UID: ${firebaseUser.uid}`
      );

      toast({
        title: "Signup Successful!",
        description: "Your account has been created. Redirecting to profile...",
      });
    } catch (error: any) {
      console.error(
        `${pageName}: Error verifying OTP or creating profile:`,
        error
      );
      toast({
  ...
  const sendOtpButtonDisabled =
    ...
    !isRecaptchaReady ||
    !!recaptchaError;
  const verifyOtpButtonDisabled =
    !!loading ||
    otp.length !== 6 ||
    (!!phoneEmail && (!phonePassword || !phoneConfirmPassword)) ||
    (!!phonePassword &&
        ...
        <CardContent>
          <Tabs
            ...
            onValueChange={(value) => {
              ...
              console.log(
                `SIGNUP_PAGE_TABS: Auth method changed to ${newAuthMethod}`
              );
                ...
                <>
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          console.log(
                            "SIGNUP_PAGE: User clicked 'Change Number or Resend OTP'."
                          );
                          setOtpSent(false);
                          setConfirmationResult(null);
                          setOtp("");
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="phoneBasedEmailSignup">
                      Email (Optional, to link with your phone account)
                    </Label>
                    <Input
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="phoneBasedPasswordSignup">
                      Password (Optional, if email provided)
                    </Label>
                    <Input
                      id="phoneBasedPasswordSignup"
                      type="password"
                      placeholder="••••••••"
                      value={phonePassword}
